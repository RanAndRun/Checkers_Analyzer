...
def where_can_move(tile: Tile) -> list[Tile]:
    ...
    if type(piece) is Pawn:
        if piece.color == EColor.white:
            print("white")
            if (x + 1 < len(tiles) and y + 1 < len(tiles[0]) and
                    type(Board.get_pawn_from_tile(tiles[x + 1][y + 1])) != Pawn):
                possible_tiles.append(tiles[x + 1][y - 1])

            if (x - 1 >= 0 and y + 1 < len(tiles[0]) and
                    type(Board.get_pawn_from_tile(tiles[x - 1][y + 1])) != Pawn):
                possible_tiles.append(tiles[x - 1][y - 1])

        else:
            if (x + 1 < len(tiles) and y - 1 >= 0 and
                    type(Board.get_pawn_from_tile(tiles[x + 1][y - 1])) != Pawn):
                possible_tiles.append(tiles[x + 1][y + 1])
